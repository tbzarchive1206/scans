(() => {
  ...
  if (!collection) {
    document.body.innerHTML = '<p class="noscript">Collection not found. <a href="../index.html">Return to Scans Archive</a>.</p>';
    return;
  }
  ...
  function galleryCard(gallery, index) {
    ...
    imageButton.setAttribute("aria-label", `OPEN GALLERY: ${gallery.name}`);
    if (gallery.coverId) {
      ...
      placeholder.textContent = "PDF / FOLDER";
    ...
    count.textContent = `${number(gallery.imageCount)} SCANS`;
    ...
    openButton.textContent = "OPEN GALLERY →";
    const source = document.createElement("a");
    source.href = driveFolder(gallery.folderId);
    source.target = "_blank";
    source.rel = "noopener noreferrer";
    source.textContent = "OPEN FOLDER ↗";
  ...
  function photoCard(image) {
    ...
    picture.src = thumb(image.id, 1600);
  ...
  function renderOpenGallery() {
    ...
    $("#dialogKicker").textContent = `${collection.nameEn} · ${number(gallery.imageCount)} SCANS`;
    $("#dialogDrive").href = driveFolder(gallery.folderId);
    const pdfs = $("#pdfLinks");
    pdfs.replaceChildren();
    pdfs.hidden = gallery.pdfs.length === 0;
    gallery.pdfs.forEach((pdf, index) => {
      const link = document.createElement("a");
      link.href = view(pdf.id);
      link.target = "_blank";
      link.rel = "noopener noreferrer";
      link.textContent = `PDF VERSION${gallery.pdfs.length > 1 ? ` ${index + 1}` : ""} ↗`;
      pdfs.append(link);
    });
    ...
    gallery.groups.forEach((group) => {
      ...
      heading.textContent = `${group.name} / ${number(group.images.length)} SCANS`;
      const grid = document.createElement("div");
      grid.className = "photo-grid";
      group.images.forEach((image) => grid.append(photoCard(image)));
  ...
  function openGallery(id) {
    state.current = collection.galleries.find((gallery) => gallery.id === id);
  ...
  document.title = `${collection.nameEn} — Scans Archive`;
  ...
  outline.textContent = "SCANS";
  title.append(solid, outline);
  const length = solid.textContent.length;
  $(".collection-masthead").classList.toggle("title-long", length > 17);
  $(".collection-masthead").classList.toggle("title-very-long", length > 23);
  $("#galleryCount").textContent = number(collection.galleryCount);
